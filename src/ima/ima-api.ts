...
export async function listKnowledge(
  ...
  const items: KnowledgeListItem[] = (data?.knowledge_list ?? []).map((entry: any) => {
    ...
    if (folderId) {
      return { kind: 'folder' as const, title: entry.name ?? entry.title ?? '', folderId }
    }
 ...
 */
export async function searchKnowledgeMerged(rawQuery: string): Promise<SearchResult> {
  ...
  const take = (item: KnowledgeItem): void => {
    if (seenIds.has(item.media_id) || seenTitles.has(item.title)) return
  ...
  // 第一轮:各通道最高质量条目
  for (const item of noteHighlights.slice(0, MERGE_CONFIG.noteHighlightLimit)) take(item)
  for (const item of pdfItems.slice(0, MERGE_CONFIG.pdfContentLimit)) take(item)
  for (const item of noteBodies.slice(0, MERGE_CONFIG.noteBodyLimit)) take(item)
  for (const item of wikiRanked.slice(0, MERGE_CONFIG.wikiLimit)) take(item)
  ...
  for (const item of rest) {
    if (merged.length >= MERGE_CONFIG.totalLimit) break
    take(item)
  }

  console.log(
    `[ima] 三通道检索:note=${noteHits.size}, pdf=${pdfHits.length}, wiki=${wikiHits.size}, 合并后=${merged.length}`
  )
