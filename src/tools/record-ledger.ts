...
const SCENE_COLUMNS: Record<LedgerScene, string[]> = {
  inspection: ['池号', '巡检时间', '巡检人', '鱼群状态', '症状描述', '严重程度', 'AI诊断', '处置建议', '知识来源', '是否预警'],
  water_quality: ['池号', '检测时间', '检测人', '溶氧(mg/L)', '氨氮(mg/L)', 'pH值', '亚硝酸盐(mg/L)', '二氧化碳(mg/L)', '硝酸盐(mg/L)', '水色描述', 'AI分析', '异常标记'],
  medication: ['池号', '用药时间', '用药人', '药品名称', '用药剂量', '用药方式', '用药原因', '备注'],
  feeding: ['池号', '喂食时间', '喂食人', '饲料种类', '投喂量(kg)', '摄食情况', '备注'],
  temperature: ['池号', '测量时间', '测量人', '水温(℃)', '棚温(℃)', '备注'],
  death: ['池号', '汇报时间', '汇报人', '死亡数量', '死亡状态', '死鱼外观', 'AI分析', '预警级别', '是否通知负责人'],
  dissection: ['池号', '汇报时间', '汇报人', '解剖器官', '异常信号', 'AI辅助判断', '备注']
}
...
export const recordLedger = defineTool({
  ...
  description: '将巡检记录写入飞书多维表格(仅追加)。scene 选择表格,池号必填;缺池号时返回追问。',
  parameters: {
    scene: {
      type: 'string',
    ...
    reporter: { type: 'string', description: '上报人(工人姓名)' }
  },
  output: {
    schema: {
      type: 'object',
  ...
  async execute(args) {
    ...
    const recordFields = buildFields(scene, args, poolId)
    ...
    try {
      ...
      return { success: false, message: `写入异常:${error instanceof Error ? error.message : String(error)}` }
    }
  }
})
 ...
 * 组装飞书表格字段
 */
function buildFields(scene: LedgerScene, args: LedgerArgs, poolId: string): Record<string, unknown> {
