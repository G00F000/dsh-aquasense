...
export const recordLedger = defineTool({
  ...
  async execute(args) {
    ...
    const frozenFields = args.fields as Record<string, unknown> | undefined
    ...
    let mutableFields: Record<string, unknown> | undefined = frozenFields ? { ...frozenFields } : undefined
...
async function buildFields(scene: LedgerScene, args: LedgerArgs, poolId: string, reporterName: string, resolvedFields?: Record<string, unknown>): Promise<Record<string, unknown>> {
