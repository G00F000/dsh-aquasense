...
type BlockType = 'temperature' | 'feeding' | 'medication' | 'unknown'

// ─── 池号归一化 ───

/** 将口语池号归一化为配置格式:"1号池"→"池1","一号池"→"池1" */
function normalizePoolRef(raw: string): string {
  const digitMap: Record<string, string> = { '一': '1', '二': '2', '三': '3', '四': '4', '五': '5', '六': '6', '七': '7', '八': '8', '九': '9', '十': '10' }
...
function identifyBlockType(block: string): BlockType {
  const head = block.slice(0, 20)
  if (/水温|棚温|温度/.test(head)) return 'temperature'
  if (/喂食|投喂|吃料|摄食|饲料/.test(head)) return 'feeding'
  if (/拌药|用药|药品|药量|拌料/.test(head)) {
    // 区分标题行(含冒号/列表)和上下文行(如"拌药的第三天")
    // 上下文行:仅含拌药关键词+非数据内容,不应触发新块
    if (/拌药的第/.test(head)) return 'unknown'
    return 'medication'
  }
  return 'unknown'
}
...
function normalizeChineseNum(s: string): string {
 ...
 */
function parseTemperature(block: string, allPools: string[]): LedgerEntry[] {
 ...
 *       "吃食情况: 秒光"
 */
function parseFeeding(block: string, allPools: string[]): LedgerEntry[] {
  ...
  const feedEntries: Array<{ pool: string; amount: number }> = []
 ...
 *  - 提取"第N天"类上下文作为备注
 */
function parseMedication(block: string, _allPools: string[]): LedgerEntry[] {
  ...
  const meds: Array<{ name: string; dose: string }> = []

  for (const line of lines) {
    const trimmed = line.trim()
    if (!trimmed) continue
    // 跳过标题行(如"拌药情况：")
    if (MED_HEADER_RE.test(trimmed)) continue
    ...
    // 跳过"暂无" / "无" / "暂缺"
    if (SKIP_DOSE_RE.test(dose)) continue
    meds.push({ name, dose })
  }
 ...
 */
export function parseDailyReport(text: string, pools?: string[]): ParseResult {
