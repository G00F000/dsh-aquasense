...
const CANVAS_MODULE: string = '@napi-rs/canvas'
/** OCR 引擎(可选依赖;同上用变量名动态 import,未安装时由 loadTesseract 给出安装提示) */
const TESSERACT_MODULE: string = 'tesseract.js'
...
export function parseOcrArgs(argv: string[]): OcrOptions {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    switch (flag) {
      ...
      case '--scale': {
        const value = takeNumber()
        if (value !== null) options.scale = value
        break
      }
      case '--lang':
...
export function readScannedTargets(cachePdfDir: string): ScannedTarget[] {
  ...
  for (const filename of readdirSync(cachePdfDir).sort()) {
    ...
    if (!head.startsWith(SCANNED_MARK_PREFIX)) continue
...
function loadTitleMap(cacheRoot: string): Record<string, string> {
  const titles: Record<string, string> = {}
  const meta = getPdfIndexMeta(join(cacheRoot, 'pdf-index'))
  for (const pdf of meta?.pdfs ?? []) {
    if (pdf.title) titles[pdf.mediaId] = pdf.title
  }
  return titles
}
...
function pageFileName(pageNo: number): string {
  return `${PAGE_FILE_PREFIX}${String(pageNo).padStart(4, '0')}.txt`
}

function loadState(dir: string): OcrCheckpointState | null {
  try {
    return JSON.parse(readFileSync(stateFilePath(dir), 'utf8')) as OcrCheckpointState
  } catch {
    return null
  }
}

function saveState(dir: string, state: OcrCheckpointState): void {
...
export function checkpointMatches(state: OcrCheckpointState | null, params: CheckpointParams): boolean {
  if (!state) return false
  return (
    state.sourceSha256 === params.sourceSha256 &&
...
function prepareCheckpoint(
  ...
  if (checkpointMatches(existing, params)) return existing as OcrCheckpointState
...
function countCompletedPages(dir: string, pageCount: number): number {
  let count = 0
  for (let pageNo = 1; pageNo <= pageCount; pageNo++) {
    if (existsSync(join(dir, pageFileName(pageNo)))) count++
  }
  return count
}

function readAllPages(dir: string, pageCount: number): string[] {
  const pages: string[] = []
  for (let pageNo = 1; pageNo <= pageCount; pageNo++) {
 ...
 */
export function buildPublishText(pages: string[], header: string): string {
...
export function canPublish(pageCount: number, completedPages: number, smokePages: number | null): boolean {
  if (smokePages !== null) return false
...
function detectLocalLangPack(lang: string): string | null {
  try {
...
export function resolveLangPath(options: OcrOptions): LangPathInfo {
  if (options.langPath) {
...
function checkLangPathUsable(langPath: string | null, lang: string): void {
  if (!langPath || /^[a-z][a-z0-9+.-]*:\/\//i.test(langPath)) return
  const found = [`${lang}.traineddata`, `${lang}.traineddata.gz`].some((name) => existsSync(join(langPath, name)))
  if (!found) {
    console.warn(
      `[ocr] 警告:${langPath} 下未找到 ${lang}.traineddata(.gz);需 ${LANG_PACK_DIR} 版本的 chi_sim 语言包,加载可能失败`
    )
  }
}
...
const canvasImport = async (): Promise<any> => import(CANVAS_MODULE)

/** 创建 OCR worker:整批复用同一 worker(lang 与 psm 只初始化一次) */
async function createOcrWorker(tesseract: any, options: OcrOptions, langPath: string | null): Promise<any> {
...
interface PdfUrlInfo {
  url: string
  headers?: Record<string, string>
}

/** 解析 get_media_info 的下载链接(签名 URL + 必须随请求携带的鉴权头) */
function resolveUrlInfo(info: unknown): PdfUrlInfo | null {
  const urlInfo = (info as { url_info?: { url?: unknown; headers?: unknown } } | undefined)?.url_info
  if (!urlInfo || typeof urlInfo.url !== 'string' || !urlInfo.url.startsWith('http')) return null
  const headers =
    urlInfo.headers && typeof urlInfo.headers === 'object' ? (urlInfo.headers as Record<string, string>) : undefined
...
async function downloadPdf(urlInfo: PdfUrlInfo, label: string): Promise<Buffer> {
  ...
  for (let attempt = 1; attempt <= DOWNLOAD_ATTEMPTS; attempt++) {
    try {
      ...
      lastError = error
      if (error instanceof UnretryableError) break
  ...
  if (lastError instanceof UnretryableError) throw lastError
...
async function recognizePage(worker: any, image: Buffer, label: string, pageNo: number): Promise<string> {
  let lastError: unknown = null
  for (let attempt = 1; attempt <= RECOGNIZE_ATTEMPTS; attempt++) {
    try {
      ...
      lastError = error
      if (attempt < RECOGNIZE_ATTEMPTS) await sleep(RETRY_SLEEP_MS)
    }
  }
...
interface BookOutcome {
  mediaId: string
  title: string
  status: 'published' | 'smoke'
  pages: number
  /** 发布文本字数(去字间空格后) */
  cleanChars: number
  /** 原始识别字数(含字间空格,用于验证去空格规则生效) */
  rawChars: number
}
...
function bookLabel(mediaId: string, title: string): string {
  if (title && title !== mediaId) return `《${title}》`
...
async function processTarget(
  ...
  try {
    ...
    if (pageCount <= 0) throw new Error(`${label} 页数为 0,无法 OCR`)
    ...
    for (let pageNo = 1; pageNo <= targetPages; pageNo++) {
      ...
      console.log(
        `[ocr] ${label} ${pageNo}/${targetPages} 页(${(perPage / 1000).toFixed(1)}s/页,剩余约 ${formatEta(remaining)})`
      )
    }
    ...
    console.log(
      `[ocr] ${label} 发布:${cachePath}(${pageCount} 页,${formatWan(cleanChars)};去字间空格前 ${formatWan(rawChars)})`
    )
...
/** 字数格式化(万为单位,日志用) */
function formatWan(chars: number): string {
...
async function main(): Promise<void> {
  ...
  try {
    for (const target of targets) {
      ...
      try {
        ...
        if (!urlInfo) throw new Error('get_media_info 未返回下载链接(url_info.url 缺失)')
        processed++
  ...
  console.log('[ocr] ===== 汇总 =====')
  console.log(
    `[ocr] 待办 ${targets.length} 本,处理 ${processed} 本(发布 ${published.length},冒烟 ${smoked.length},失败 ${failures.length}${filtered > 0 ? `,--only 过滤 ${filtered}` : ''})`
  )
  if (published.length > 0) {
    console.log(
      `[ocr] 覆写缓存 ${published.length} 本:${totalPages} 页,${formatWan(totalClean)}(去字间空格前 ${formatWan(totalRaw)})`
    )
  }
