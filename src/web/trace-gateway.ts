 ...
 */
export function resolveTraceRoute(pathname: string): TraceRoute {
  ...
  if (rest === '' || rest === '/') return { kind: 'page', page: 'list' }
...
function parsePositiveInt(raw: string | null, name: string, fallback: number | undefined): number | undefined {
...
export function parseTrendDays(params: URLSearchParams): number {
...
/** 页面路由 → HTML 文件名 */
const PAGE_FILE: Record<TracePage, string> = {
 ...
 */
export async function readTracePage(page: TracePage): Promise<string> {
 ...
 */
export function createTraceHandler(deps: TraceServerDeps): (req: IncomingMessage, res: ServerResponse) => Promise<void> {
...
async function servePage(deps: TraceServerDeps, page: TracePage, res: ServerResponse): Promise<void> {
  let html: string
  try {
