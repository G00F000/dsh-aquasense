 ...
 */
export async function findPendingAgentRecord(pool: string, sinceIso: string): Promise<string[]> {
  const index = await readIndex()
 ...
 * @returns 是否回填成功
 */
export async function patchReportAgent(id: string, agent: AgentTraceData): Promise<boolean> {
  ...
  const adviceCall = agent.calls.find(
    (c) => c.tool === 'aquasense_advice' && c.status === 'ok' && c.meta && typeof c.meta.retrieve === 'object'
  )
  if (adviceCall?.meta) {
    const retrieve = adviceCall.meta.retrieve as Partial<SpanRetrieve> | undefined
    if (retrieve && (!record.span_retrieve || (!record.span_retrieve.query && retrieve.query))) {
      record.span_retrieve = retrieve as SpanRetrieve
    }
  ...
  console.log(
    `[aquasense-trace] 群聊 Agent 决策链已回填: ${id}, turn ${agent.turn}, ` +
 ...
 */
export function refreshIndexSummary(record: AnalysisRecord): Promise<void> {
  const task = writeChain.then(async () => {
    const index = await readIndex()
