...
/** 飞书消息体(msg_type: interactive=卡片, text=降级文本) */
...
let activeConfig: RemindConfig | null = null
let currentPlan: PushPlan | null = null
...
function remindDir(): string {
  const dir = join(resolveCacheRoot(), 'remind')
  mkdirSync(dir, { recursive: true })
  return dir
}

function loadConfig(): RemindConfig {
...
function readConfigFile(): Partial<RemindConfig> | null {
  const path = join(remindDir(), 'config.json')
  if (!existsSync(path)) return null
  try {
    const parsed = JSON.parse(readFileSync(path, 'utf8')) as Partial<RemindConfig>
...
function parseEnvTasks(raw: string | undefined): unknown {
...
export function normalizeRemindTask(input: unknown): RemindTask | null {
  ...
  if (!isValidHHMM(time) || !task) return null
  return { time, task }
}

/** 校验任务列表:非法条目跳过并计数;合法条目按 time 升序 */
function sanitizeTasks(raw: unknown): { tasks: RemindTask[]; skipped: number } {
  if (!Array.isArray(raw)) return { tasks: [], skipped: raw === undefined ? 0 : 1 }
...
function parseCronTime(expr: string): string | null {
  ...
  if (!minMatch || !hourMatch) return null
...
function sentPath(date: string): string {
  return join(remindDir(), `sent-${date}.json`)
}

function buildPlanItems(overviewTime: string | null, tasks: RemindTask[]): PlanItem[] {
  const items: PlanItem[] = []
  if (overviewTime) items.push({ time: overviewTime, type: 'overview' })
  for (const t of tasks) items.push({ time: t.time, type: 'task', task: t.task })
...
function loadOrBuildPlan(date: string, overviewTime: string | null, tasks: RemindTask[]): PushPlan {
  ...
  if (existsSync(planPath(date))) {
    try {
      const existing = JSON.parse(readFileSync(planPath(date), 'utf8')) as PushPlan
  ...
  const plan: PushPlan = { date, items: expected }
...
function loadSentKeys(date: string): Set<string> {
  if (!existsSync(sentPath(date))) return new Set()
  try {
    const parsed = JSON.parse(readFileSync(sentPath(date), 'utf8')) as { sent?: string[] }
...
/** 卡片 A: 每日任务总览(原型 1) */
function buildOverviewCard(plan: PushPlan): object {
...
function buildTaskCard(item: PlanItem, plan: PushPlan): object {
  const elements: object[] = [
...
/** 卡片 C: 异常预警(原型 4) */
function buildAbnormalCard(input: AbnormalAlertInput): object {
...
function buildFallbackText(item: PlanItem, plan: PushPlan): string {
  ...
  const remaining = plan.items
    ...
    .slice(0, REMAINING_PREVIEW_COUNT)
    .map((i) => `${i.time} ${i.task}`)
  if (remaining.length > 0) lines.push(`💡 今日剩余: ${remaining.join('、')}`)
  lines.push('✅ 完成后拍照发到本群,自动记录台账')
  return lines.join('\n')
}

async function sendToFeishu(message: FeishuMessage, group?: string): Promise<void> {
...
async function pushWithRetry(message: FeishuMessage, label: string): Promise<boolean> {
  try {
...
async function tick(): Promise<void> {
  ...
  try {
    ...
    for (const item of currentPlan.items) {
      ...
      if (sentKeys.has(key) || processedKeys.has(key)) continue
...
function rollNewDay(date: string): void {
 ...
 */
export function setupS9Reminder(ctx?: Context): void {
  stopS9Reminder()
 ...
 */
export async function pushAbnormalAlert(input: AbnormalAlertInput): Promise<void> {
  try {
    const config = activeConfig ?? loadConfig()
    if (!config.enabled || !config.group) return

    await pushWithRetry(
      { msgType: 'interactive', content: JSON.stringify(buildAbnormalCard(input)) },
      `预警卡片 ${input.poolId ?? '未知池'} ${input.cls}(${input.severity})`
    )
  } catch (error) {
...
/** 读取当前生效配置(文件 > 环境变量) */
export function getRemindConfig(): RemindConfig {
  return loadConfig()
}
 ...
 */
export function saveRemindConfig(input: RemindConfigInput): RemindConfig {
...
export function getRemindStatus(): RemindStatus {
  const date = today()
