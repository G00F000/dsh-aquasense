 ...
 *  - lark://xxx
 */
export function isFeishuInternalUrl(url: string): boolean {
  if (!url) return false
 ...
 */
export async function downloadFeishuImage(messageKey: string): Promise<{ data: string; mimeType: string } | null> {
  try {
    const token = await getFeishuToken()
    // 使用飞书API下载消息资源
 ...
 * @returns 图片数据(base64)和MIME类型，失败返回null
 */
export async function downloadImageWithFallback(url: string, retryCount = 1): Promise<{ data: string; mimeType: string } | null> {
  // 1. 优先尝试HTTP/HTTPS下载
