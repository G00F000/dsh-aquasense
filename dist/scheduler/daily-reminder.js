...
async function loadTodayManual() {
    const date = today();
    ...
    const manual = { date, tasks: tasks.sort((a, b) => a.time.localeCompare(b.time)) };
...
async function pushWithRetry(text, label) {
    try {
        await pushText(text);
...
function buildTaskCard(task, remaining) {
...
function pushMissingManualAlert() {
