...
// ========== 写入 ==========
/** 写入单条完整记录(目录不存在时自动创建) */
export async function writeReport(record) {
 ...
 */
export function updateIndex(record) {
...
export async function patchReportAgent(id, agent) {
    ...
    const analyzeCall = agent.calls.find((c) => c.tool === 'aquasense_analyze' && c.status === 'ok' && c.meta);
    if (analyzeCall?.meta && record.span_analyze) {
        ...
        if (!record.span_analyze.data_completeness && isValidCompleteness(meta.data_completeness)) {
            record.span_analyze.data_completeness = meta.data_completeness;
        }
...
export async function rebuildIndex() {
    ...
    const index = { version: 1, records: records.map(toSummary) };
    await fs.mkdir(reportsDir(), { recursive: true });
    await fs.writeFile(indexFile(), JSON.stringify(index, null, 2), 'utf-8');
 ...
 * @returns 清理条数
 */
export async function cleanupOldReports(daysToKeep = 90) {
...
export async function queryIndex(query) {
    ...
    const filtered = index.records.filter((r) => {
        if (query.pool && r.pool !== query.pool)
            return false;
        if (query.cls && r.cls !== query.cls)
            return false;
        // ID 形如 RPT-20260917-100532,第 4..12 位为本地日期
        if (dateKey && r.id.slice(4, 12) !== dateKey)
            return false;
        if (query.low_confidence && r.confidence >= 0.8)
            return false;
        if (query.has_error && !((r.agent_retries !== undefined && r.agent_retries > 0) || r.cls === 'unknown'))
            return false;
        return true;
    });
    return {
 ...
 */
export async function computeTrend(pool, days) {
    const index = await readIndex();
    ...
    for (const r of scoped) {
        if (r.cls === 'normal' || r.cls === 'early' || r.cls === 'disease')
            distribution[r.cls] += 1;
        else
