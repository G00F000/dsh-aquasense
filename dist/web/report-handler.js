...
/** 状态中文名(进度文案) */
const CLS_NAME = { normal: '正常', early: '前兆', disease: '发病', unknown: '未知' };
/** 巡检表中文名(详情页展示 target_table) */
const INSPECTION_TABLE_NAME = '巡检记录表';
/** 台账工具 exec 参数桩:H5 管线无 Agent 会话上下文,recordLedger 实现不使用 exec */
const LEDGER_STUB_EXEC = {};
// ========== 表单解析 ==========
/** 读取请求体的最大字节数(超限 413) */
export async function parseReportForm(req, maxBytes = MAX_BODY_BYTES) {
...
/** 更新进度(仅前进,不回退) */
function setProgress(job, progress, step) {
...
function pickLedgerOutcome(result) {
 ...
 */
export function startReportJob(input, uploadDurationMs, deps) {
    cleanupJobs();
 ...
 */
export async function runReportPipeline(job, input, uploadDurationMs, depsOverride = {}) {
    const deps = { ...defaultPipelineDeps(), ...depsOverride };
    ...
    job.pool = input.pool;
    console.log(`[aquasense-trace] H5 汇报开始: ${tracer.id} (${input.pool}, ${input.reporter}, ${input.images.length} 张)`, input.description ? `描述:${input.description.slice(0, 200)}` : '');
 ...
 */
export async function handleReportHttp(req, res, deps) {
    try {
...
export async function handleReportPage(req, res) {
    try {
        if ((req.method ?? '') !== 'GET') {
            writeEnvelope(res, 405, fail(405, 'method-not-allowed', '仅支持 GET').body);
            return;
 ...
 */
export function installReportWeb(ctx) {
