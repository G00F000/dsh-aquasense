...
function probeJieba() {
    ...
    void import(moduleName)
        ...
        if (typeof mod?.cut === 'function') {
            jiebaCut = (text) => mod.cut(text, true);
...
function tokenizeChinese(text) {
    ...
    if (jiebaCut) {
        for (const word of jiebaCut(text)) {
            ...
            if (trimmed.length > 0 && !STOP_WORDS.has(trimmed))
                terms.add(trimmed);
        }
    }
    else {
        for (const term of ngramTokenize(text))
            terms.add(term);
    }
    for (const word of lexiconHits(text))
        terms.add(word);
    return [...terms];
}
...
function ngramTokenize(text) {
    ...
    for (let i = 0; i < clean.length - 1; i++) {
        ...
        if (!hasStopChar(bigram))
            terms.add(bigram);
    }
    for (let i = 0; i < clean.length - 2; i++) {
        const trigram = clean.slice(i, i + 3);
        if (!hasStopChar(trigram))
            terms.add(trigram);
    }
    return [...terms];
}
 ...
 */
export async function buildPdfIndex(cachePdfDir, indexDir, options = {}) {
    ...
    console.log(`[pdf-index] 加载 ${pdfTexts.length} 个有效 PDF 文本(跳过标记 ${skippedMarks},过短 ${skippedShort})`);
    ...
    if (cachedIndex?.dir === indexDir)
        cachedIndex = null;
    console.log(`[pdf-index] 构建完成: ${allChunks.length} 切片, ${pdfMeta.length} PDF, 耗时 ${((Date.now() - startedAt) / 1000).toFixed(1)}s, chunks ${formatBytes(chunksJson.length)}`);
    return meta;
}
/** 状态标记文件判定:内容以 [ 开头(OCR 文本头标记除外,那是真实内容) */
function isStatusMark(text) {
...
function splitIntoBlocks(text) {
    const blocks = [];
    const push = (segment, segStart) => {
        const leading = segment.length - segment.trimStart().length;
        const trimmed = segment.trim();
        if (trimmed.length > 0)
            blocks.push({ text: trimmed, start: segStart + leading });
    };
    if (text.includes('\f')) {
        let cursor = 0;
...
function chunkBlocks(blocks, mediaId, title, pageMarked) {
    const chunks = [];
    let chapter = null;
...
function extractTitleFromContent(text, mediaId) {
    for (const line of text.split('\n').slice(0, 5)) {
        const trimmed = line.trim();
        ...
        if (/[。！？：:;；]/.test(trimmed))
            continue;
        if (/^[（(]\d+[)）]/.test(trimmed))
            continue; // 专利首页头"(19)国家知识产权局"等
        if (detectChapter(trimmed))
            continue; // 章节标题不是书名
        return trimmed;
    }
    return mediaId;
 ...
 * @param indexDir 索引目录
 */
export function searchPdfContent(query, indexDir) {
    const startedAt = Date.now();
    ...
    for (let i = 0; i < index.chunks.length; i++) {
        ...
        for (const term of queryTerms) {
            ...
            if (count > 0) {
                score += count;
                if (first >= 0 && (matchPos < 0 || first < matchPos))
                    matchPos = first;
                (matchedTerms ??= []).push(term);
            }
        }
        if (matchedTerms)
            scored.push({ chunkIdx: i, score, matchedTerms, matchPos: matchPos < 0 ? 0 : matchPos });
    }
    ...
    for (const hit of scored) {
        ...
        if (results.length >= SEARCH_TOP_K)
            break;
    }
    console.log(`[pdf-index] 检索"${query.slice(0, 30)}" -> ${results.length} 条命中,耗时 ${Date.now() - startedAt}ms`);
    return results;
}
...
export function isPdfIndexReady(indexDir) {
...
export function getPdfIndexStatus(indexDir) {
    const meta = getPdfIndexMeta(indexDir);
    if (!meta)
        return 'not_found';
    const builtAt = new Date(meta.builtAt).getTime();
...
/** 获取索引元数据(不加载完整索引) */
export function getPdfIndexMeta(indexDir) {
    const indexPath = join(indexDir, 'index.json');
 ...
 */
export function pdfHitToKnowledgeItem(hit) {
...
/** 围绕最早命中的关键词截取上下文并标记 */
function markHighlightTerms(text, terms) {
    let earliest = -1;
