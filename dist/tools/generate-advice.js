 ...
 */
export async function retrieveKnowledge(analysis) {
 ...
 */
export async function generateAdviceInternal(analysis, knowledge, excerpts) {
    const knowledgeRefs = knowledge && knowledge.items.length > 0
...
function buildReasoning(analysis, excerpts, hitCount) {
    ...
    if (excerpts.length > 0) {
        ...
        const sourceNote = pdfCount > 0 && noteCount > 0
