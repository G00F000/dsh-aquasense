...
> **入口实现注（v1.7）**：设置座位旁的一级动作注册面已定位为 `sidebar.footer.action` 列表槽（由 `@deepseek-ai/dsh-client-ui-sidebar` 声明，渲染于设置座位旁的 footerActions 容器），原型图「🐟 智慧渔业按钮 / ⚙ 设置」的层级得以原生落地：点击按钮在会话列上打开**独立配置页**（不跳转设置页），页顶为二级标题「每日任务提醒」+ 右上角关闭按钮，布局对齐 SkillHub 插件广场。**v1.8 起为唯一入口**（设置卡片入口已移除；另修复页脚动作容器单行 flex 挤压问题——经 `:has()` 允许换行，「插件广场 / 智慧渔业 / 设置」各占整行）。
...
> **配置页实现注（v1.7）**：配置页（`src/client/AquaConfig.tsx`）表单由 `src/client/RemindForm.tsx` 提供（`useRemindConfig` 数据层 + `RemindForm` 视图，v1.8 起为唯一使用方）；侧栏按钮宽态显示「智慧渔业」文字、56px 收起轨道仅显示水波线性图标；交互（Esc / 点击面板外关闭、随会话列尺寸自适应）与版式（顶栏二级标题 + 右上角 32×32 关闭按钮、内容区 760px 上限）对齐 SkillHub 插件广场页面。

> **入口更名注（v1.9）**：侧栏一级入口由「🐟 AquaSense 配置」更名「智慧渔业」（`entry.label`：智慧渔业 / Smart Fishery），图标由 emoji 换为与插件广场（SkillHub）同风格的**内联线性 SVG**（`WavesIcon`（三道水波）：16×16 视窗、`fill="none"`、`stroke="currentColor"`、`strokeWidth="1.4"`；尺寸档 16px、56px 收起轨道 18px，对齐插件广场 `PlazaIcon` 描边风格，随按钮文字色自动着色）。
