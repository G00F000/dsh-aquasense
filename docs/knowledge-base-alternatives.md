...
| ⑤ 引用定位 | 页码 + 原文片段 + 高亮 | ⚠️ 部分:原文片段 + 高亮 ✅,页码已透出到引用(knowledge_excerpt);但页码仅含分页符(\f)的缓存(OCR 册)可取,chapter 实测几乎全为 null |
| ⑥ 预整理(可选) | Wiki 模式、多索引整理(NotebookLM 式) | 部分:IMA 笔记(二次汇总,保真度约 30%) |

> ⑤ 实测口径:页码为 1-based 缓存页序,仅含分页符(\f)的缓存可取——分页符由 OCR 批处理写入,原生 unpdf 提取无页边界 → `page=null`(不臆造);章节检测命中率低,`chapter` 几乎全为 null。页码经 `KnowledgeItem.locator` 透出至 `knowledge_excerpt`(格式 `《标题》第N页:「摘录」`)。

**结论**:现状已是"迷你 IMA";缺口集中在 ③④ 两个环节,⑤ 的页码溯源受数据层(分页符)制约,⑥ 决定体验上限。
